...
export default function UploadPage() {
  ...
  const extractTextFromFile = async (file: File): Promise<string> => {
    ...
    // Note: PDF text extraction requires a heavy client-side library like pdf.js
    // If you need PDFs, it's highly recommended to send the file to a backend API instead.
...
Once all these steps are performed, format the data EXACTLY in this JSON structure:
{
  ...
  "risky_clauses": [["12345", 3], ["67890", 2]]
      ...
      console.log("Analysis complete:", parsedContent);
  ...
  return (
    <div className="mx-auto max-w-3xl px-6 py-10 sm:px-8 pb-24">
      ...
      {analysisResult && (
        <div className="mt-8 rounded-xl bg-slate-900 p-6 text-sm text-green-400 overflow-x-auto">
      ...
      {isReadyToAnalyze && !analysisResult && (
        <div className="fixed bottom-8 right-8 flex transform justify-center">
          <button
            ...
            className="flex items-center gap-2 rounded-full bg-sky-600 px-8 py-4 text-base font-semibold text-white shadow-xl transition-all hover:-translate-y-1 hover:bg-sky-700 hover:shadow-2xl disabled:opacity-50 disabled:hover:translate-y-0 dark:bg-sky-500 dark:hover:bg-sky-600"
          >
            {isAnalyzing ? (
